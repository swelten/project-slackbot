...
Nachdem ein Projekt-Channel (`prj_*`) angelegt wurde, achtet der Bot auf neue Datei-Uploads. Der jeweilige Uploader bekommt eine ephemere Nachfrage, ob die Datei in den zum Projekt gehörenden OneDrive-Ordner verschoben werden soll (Link aus der Notion-Property). Bei Zustimmung lädt der Bot die Slack-Datei (`files:read`-Scope erforderlich) über Microsoft Graph direkt in den Ordner – kleine Dateien bis 4 MB via Direkt-Upload, größere Dateien über Upload-Sessions. Die OneDrive-Umgebungsvariablen müssen dafür gesetzt sein.
...
- Immer erneut installieren, nachdem neue Scopes (z. B. `files:read` für OneDrive-Uploads) ergänzt wurden, sonst stellt Slack die neuen Berechtigungen nicht zur Verfügung.
